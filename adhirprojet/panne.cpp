...
panne::panne(int idvehicule, int chauvehicule  ,QString typepanne ,QString refpanne)
{
...
QString panne::get_refpanne(){return  refpanne;}
...
query.prepare("INSERT INTO panne (ID_VEHICULE, CIN_CHAUFFEUR ,TYPE_PANNE,REF_PANNE) "
                    "VALUES (:idvehicule,  :chauvehicule, :typepanne , :refpanne)");
...
query.bindValue(":refpanne", refpanne);
...
bool panne::supprimer(int idvehicule)
{
    QSqlQuery query;
      ...
      query.bindValue(":idvehicule", res);
      return    query.exec();
